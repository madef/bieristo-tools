...
class ViewPressure {
    ...
    this.view = new Brique(`<div class="flex flex-col gap-4">
        <div class="flex flex-wrap gap-2 p-2">
          <div class="w-full flex flex-col grow gap-1">
            ...
            <div class="relative grow rounded-md shadow-sm flex gap-2">
              <div class="flex grow w-full items-center gap-2 rounded-md border border-white pr-2 group hover:border-amber-500 focus-within:border-amber-500">
                <select autocomplete="off" id="style" class="grow rounded-md py-1 px-2 w-full text-lg focus:outline-none bg-transparent" data-var="style">
                </select>
              </div>
            </div>
          </div>
          <div class="w-full sm:w-1/4 flex flex-col grow gap-1">
          ...
          <div data-var="result" class="hidden flex w-full gap-2">
  ...
  updateResult (status) {
    ...
    this.view.append(
      'totalMin',
      new Brique(`<input type="text" readonly autocomplete="off" id="totalMin" class="w-1/2 rounded-md bg-transparent grow text-lg focus:outline-none" data-var="totalMinInput" value="${totalMin}">
  <div class="text-nowrap">${totalUnit}</div>
  <button class="hover:text-amber-500" data-var="copyTotalMin" title="${Translator.__('Generic:Action:copy')}" aria-label="${Translator.__('Generic:Action:copy')}">
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
      <path stroke-linecap="round" stroke-linejoin="round" d="M15.666 3.888A2.25 2.25 0 0 0 13.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 0 1-.75.75H9a.75.75 0 0 1-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 0 1-2.25 2.25H6.75A2.25 2.25 0 0 1 4.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 0 1 1.927-.184" />
    </svg>
  </button>
</div>`)
        .addEventListener('copyTotalMin', 'click', () => {
          navigator.clipboard.writeText(this.view.get('totalMinInput').value)
    ...
    this.view.append(
      'totalMax',
      new Brique(`<input type="text" readonly autocomplete="off" id="totalMax" class="w-1/2 rounded-md bg-transparent grow text-lg focus:outline-none" data-var="totalMaxInput" value="${totalMax}">
  <div class="text-nowrap">${totalUnit}</div>
  <button class="hover:text-amber-500" data-var="copyTotalMax" title="${Translator.__('Generic:Action:copy')}" aria-label="${Translator.__('Generic:Action:copy')}">
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
      <path stroke-linecap="round" stroke-linejoin="round" d="M15.666 3.888A2.25 2.25 0 0 0 13.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 0 1-.75.75H9a.75.75 0 0 1-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 0 1-2.25 2.25H6.75A2.25 2.25 0 0 1 4.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 0 1 1.927-.184" />
    </svg>
  </button>
</div>`)
        .addEventListener('copyTotalMax', 'click', () => {
          navigator.clipboard.writeText(this.view.get('totalMaxInput').value)
          new Confirm( // eslint-disable-line no-new
            () => {
            },
            Translator.__('Generic:Confirm:copy'),
