...
class BlockHistory {
  ...
  render () {
    ...
    const root = new Brique('<ul data-var="history" class="sm:m-2 sm:border sm:border-zinc-700 rounded"></ul>')
    root.appendTo(this.$root)

    this.history.get().forEach((historyRow, historyKey) => {
      /* eslint-disable no-new */
      const history = new Brique(`<li class="flex flex-wrap odd:bg-box even:bg-zinc-800 p-2 sm:p-4 md:justify-between items-center gap-2">
          ...
          <button class="bg-cyan-700 hover:bg-transparent hover:text-cyan-700 focus:bg-transparent focus:text-cyan-700 rounded flex gap-1 items-center p-2" data-var="copy">
            ...
            ${Translator.__('History:Action:copy')}
          </button>
