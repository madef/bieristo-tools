...
class BlockHistory {
  ...
  render () {
    ...
    const root = new Brique('<ul data-var="history" class="sm:m-2"></ul>')
    root.appendTo(this.$root)

    this.history.get().forEach((historyRow, historyKey) => {
      /* eslint-disable no-new */
      const history = new Brique(`<li class="flex flex-wrap odd:bg-white/5 p-2 sm:p-4 md:justify-between items-center gap-2">
